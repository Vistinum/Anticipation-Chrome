...
function hideTimes(){
	if(!twitch){
		...
		times = document.getElementsByClassName("player-slider player-slider--roundhandle js-player-slider")
		for(i = 0; i < times.length; i++){
			var vidTime = times[i];
			var par1 = vidTime.parentNode
			par1.removeChild(vidTime);
		...
		times = document.getElementsByClassName("info")
		for(i = 0; i < times.length; i++){
			var vidTime = times[i];
		...
		// Custom
		document.querySelectorAll(".player-seek, .preview-card-overlay, .preview-card-thumbnail__image").forEach(e => e.style.display='none');
	}
}